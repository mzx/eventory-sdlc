...
<!-- AC:BEGIN -->
- [ ] Non-allowlisted Google sign-in refused at callback with the invite-only page; no usable session; e2e-covered
- [ ] Allowlisted email and `@domain` entries both admit; invited users admit via token regardless of allowlist; zero-user bootstrap admits (all e2e)
- [ ] Unset var keeps current behavior with a startup warning; README + .env examples + prod compose updated (env actually plumbed — the EVT-20 check)
- [ ] Invite revoke is conditional (409 on already-redeemed); redeem returns the actual role; both tested
- [ ] Stale doc references corrected
